 ...
 * 
 */
@Service("accountAlertIPadConsumer")
public class AccountAlertIPadConsumer extends BaseAlertConsumer implements
		MessageListener {

	@Autowired
	@Qualifier("accountAlertIpadQueue")
	Queue accountAlertIpadQueue;

	@Autowired
	@Qualifier("notificationProxy")
	private ISender notificationProxy;
	
	private final static Logger logger = Logger.getLogger(AccountAlertIPadConsumer.class);

	@PostConstruct
	public void init() throws Exception {
		super.init();
		MessageConsumer notificationsQueueConsumer = notificationsQueueSession
				.createConsumer(accountAlertIpadQueue);
