 ...
 * 
 */
@Service("schedulerAlertIPadConsumer")
public class SchedulerAlertIPadConsumer extends BaseAlertConsumer implements
		MessageListener {

	@Autowired
	@Qualifier("schedulerAlertIpadQueue")
	Queue schedulerAlertIpadQueue;

	@Autowired
	@Qualifier("notificationProxy")
	private ISender notificationProxy;
	
	private final static Logger logger = Logger.getLogger(SchedulerAlertIPadConsumer.class);

	@PostConstruct
	public void init() throws Exception {
		super.init();
		MessageConsumer notificationsQueueConsumer = notificationsQueueSession
				.createConsumer(schedulerAlertIpadQueue);
